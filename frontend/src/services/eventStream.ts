...
export function connectToResearchStream(
  ...
  const handleEvent = (eventName: string, event: MessageEvent) => {
    ...
    try {
      ...
      console.log("Parsed JSON pretty:", JSON.stringify(parsed, null, 2));
    } catch (error) {
      console.error("JSON parse failed:", error);

      console.log("Using raw data:", event.data);
    ...
    console.log("Complete backend payload:", parsed);

    const type = parsed?.event || parsed?.type || eventName;
    ...
    console.log("Resolved data pretty:", JSON.stringify(data, null, 2));
  ...
  const connect = () => {
    ...
    console.log("[SSE] Research ID:", researchId);
    ...
    eventSource.onmessage = (event) => {
      console.log("[SSE] DEFAULT MESSAGE EVENT");

      handleEvent("message", event);
    };

    researchEvents.forEach((eventName) => {
      eventSource?.addEventListener(eventName, (event) => {
        handleEvent(eventName, event as MessageEvent);
      });
    });
