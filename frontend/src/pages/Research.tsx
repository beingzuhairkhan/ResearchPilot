...
export function Research() {
  ...
  const { timeline, activities, completed, failed, progress } =
  ...
  useEffect(() => {
    console.log("[Research] sources:", sources);
  }, [sources]);
  ...
  }, [timeline, activities, completed, failed, progress]);

  const handleCompleted = useCallback(async () => {
    if (!id || completionHandledRef.current) {
      return;
    }
    ...
    try {
      ...
      const response = await researchApi.getResearch(id);

      console.log("[Research] completed API response:", response);
      ...
      console.error("[Research] completion refresh error:", err);
    }
  ...
  useEffect(() => {
    ...
    const activeStep = timeline.find((step) => step.status === "active");

    const nextProgress = progress;

    const nextStep = activeStep?.label ?? session.currentStep;

    if (session.progress === nextProgress && session.currentStep === nextStep) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      console.log("[Research] polling sources");
  ...
  }, [id, showContent, session?.status, fetchSources]);
  ...
  const currentProgress = progress > 0 ? progress : (session.progress ?? 0);

  const currentStep =
    timeline.find((step) => step.status === "active")?.label ??
    session.currentStep ??
    "";

  console.log("[Research] rendering", {
  ...
  return (
    <PageContainer maxWidth="xl">
      ...
      {showContent && session.sourcesFound > 0 && (
        <div className="mb-6">
      ...
      {showContent && (
        <div className="mb-6">
          <ResearchPlan plan={plan} loading={!plan} />
        </div>
      )}
      ...
      {showContent && (
        <div className="mb-6">
          <SourceList
            sources={sources}
            loading={sources.length === 0 && session.status === "researching"}
          />
        </div>
      )}
    </PageContainer>
