...
export function ResearchPlan({ plan, loading }: ResearchPlanProps) {
  if (loading) {
    return (
      <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-card">
        <h3 className="mb-1 text-sm font-semibold text-neutral-900">Research Plan</h3>
        <p className="mb-5 text-xs text-neutral-500">AI-generated research strategy</p>
 ...
 const completed = plan.data.tasks.filter((t) => t.status === 'completed').length;
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-card">
      <div className="mb-5">
        <h3 className="text-sm font-semibold text-neutral-900">Research Plan</h3>
        <p className="mt-0.5 text-xs text-neutral-500">AI-generated research strategy</p>
      </div>
      ...
      <div className="mb-5 flex items-start gap-3 rounded-xl bg-neutral-50 p-4">
        ...
        <div>
          <p className="text-xs font-medium uppercase tracking-wider text-neutral-400">Objective</p>
          <p className="mt-0.5 text-sm text-neutral-800">{plan.objective}</p>
        </div>
      </div>

      <div className="mb-4 flex items-center justify-between">
        <p className="text-xs font-medium text-neutral-500">
          {plan.data.tasks.length} research tasks
          
        </p>
        <p className="text-xs text-neutral-400">
          {completed}/{plan.data.tasks.length} completed
        </p>
      </div>
