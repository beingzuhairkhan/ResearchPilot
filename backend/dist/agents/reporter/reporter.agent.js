...
let ReporterAgent = ReporterAgent_1 = class ReporterAgent {
    constructor(llmProvider) {
        this.llmProvider = llmProvider;
        this.logger = new common_1.Logger(ReporterAgent_1.name);
    }
    async generateReport(input) {
        ...
        const sourcesList = input.sources
            ...
            .join('\n');
        const claimsContext = JSON.stringify(input.claims.map((c) => ({
            claim: c.claim,
    ...
    parseReport(content) {
        try {
            ...
            return {
                ...
                keyFindings: (parsed.keyFindings || []).map((f) => String(f)),
                recentDevelopments: (parsed.recentDevelopments || []).map((d) => String(d)),
                conflictingEvidence: [],
...
exports.ReporterAgent = ReporterAgent = ReporterAgent_1 = __decorate([
